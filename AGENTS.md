...
5. Verify Flash with a real tool loop using
   `codex -m deepseek/deepseek-flash -c 'model_reasoning_effort="max"' -a never exec
   --skip-git-repo-check 'call a shell tool exactly once …'`.
   Legacy V4 Flash and Pro identifiers must continue routing to `deepseek-flash` for resumed tasks.
...
8. DeepSeek Flash now uses `deepseek-flash` (V4.1 Flash) with native vision. Forward
   `input_image` parts in messages and tool outputs directly to DeepSeek. The single catalog
   entry keeps `input_modalities = ["text", "image"]` to permit desktop `view_image` calls.
   GPT vision descriptions and `DSCODEX_VISION_MODEL` are no longer used by the router.
...
15. Pets, plugins, skills, and MCP remain client-side. Voice uses GPT-Live and must never
    route to DeepSeek; Realtime routing compatibility remains pending PR #21 validation.
    The catalog keeps `prefer_websockets = false` and the router returns 426 for upgrades.
16. Before GPT-bound requests, remove foreign plaintext `reasoning_text` reasoning items,
    preserving native encrypted GPT reasoning. Decrypt DSCodex-prefixed compaction into assistant
    context; drop invalid DSCodex compaction. Ordinary GPT requests remain byte-for-byte intact.
    Rewritten compressed requests must lose their original content-encoding header.
