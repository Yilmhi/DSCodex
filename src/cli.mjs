...
async function restoreManualRouter(paths, port, cause, rollback) {
  ...
  if (rollback) {
    try {
      ...
      // Artifact or manager cleanup can fail after the router was stopped.
      // Still attempt recovery; start checks authenticated health before spawning.
  ...
  try {
    ...
    throw new AggregateError(
      failures,
      `Autostart failed and the previous manual router could not be restored; inspect ${paths.log}: ${failures.map((error) => error instanceof Error ? error.message : String(error)).join("; ")}`,
    );
  }
  if (failures.length > 1) {
    throw new AggregateError(
      failures,
      `Autostart failed; router availability was restored but rollback was incomplete: ${failures.map((error) => error instanceof Error ? error.message : String(error)).join("; ")}`,
    );
  }
  throw new Error(
...
Usage: dscodex <command> [--port ${DEFAULT_PORT}]
  ...
  proxy set   store the outbound proxy for GPT traffic (e.g. http://127.0.0.1:10808)
