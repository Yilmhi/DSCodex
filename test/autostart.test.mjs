...
test("failed handoff restores a manual router even when removing the service artifact fails", {
  timeout: 20_000,
  skip: process.platform !== "darwin" ? "macOS manager integration test" : false,
}, async () => {
  const codexHome = mkdtempSync(join(tmpdir(), "dscodex-autostart-restore-"));
  const port = 20_000 + Math.floor(Math.random() * 20_000);
  const paths = prepareRouterHome(codexHome, port);
  const hook = join(codexHome, "failed-launchd.cjs");
  // Intercept every launchctl invocation and isolate homedir, so this test never
  // contacts the real service manager or writes the user's LaunchAgents folder.
  writeFileSync(hook, `
    const fs = require("node:fs");
    ...
    cp.execFileSync = function(file, args, options) {
      if (file === "/bin/launchctl") {
        if (args[0] === "bootstrap") throw new Error("simulated bootstrap failure");
    ...
    fs.unlinkSync = function(path) {
      if (String(path).endsWith("com.dscodex.router.plist")) {
        throw Object.assign(new Error("simulated artifact removal failure"), { code: "EACCES" });
      }
      return unlinkSync.apply(this, arguments);
    };
    os.homedir = () => process.env.CODEX_HOME;
    syncBuiltinESMExports();
  `);
  const env = { ...routerEnv(codexHome), NODE_OPTIONS: `--require=${hook}` };
  const child = spawn(process.execPath, [CLI, "serve", "--port", String(port)], {
    env, stdio: "ignore",
  });
  try {
    await waitForFile(paths.pid, "manual router pid file");
    const result = await runCli(["autostart", "enable", "--port", String(port)], env);
    assert.notEqual(result.code, 0, `${result.stdout}\n${result.stderr}`);
    const status = await runCli(["status", "--port", String(port)], env);
    assert.equal(status.code, 0, `${status.stdout}\n${status.stderr}`);
    assert.match(status.stdout, /running/);
    assert.match(result.stderr, /router availability was restored but rollback was incomplete/);
    assert.match(result.stderr, /simulated bootstrap failure/);
    assert.match(result.stderr, /simulated artifact removal failure/);
    assert.notEqual(JSON.parse(readFileSync(paths.pid, "utf8")).pid, child.pid);
  } finally {
    await runCli(["stop", "--port", String(port)], env);
    child.kill("SIGKILL");
    rmSync(codexHome, { recursive: true, force: true });
